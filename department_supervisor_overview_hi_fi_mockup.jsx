        ...
        <div className="grid gap-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <div className="text-sm font-medium">Shift hours</div>
              <Input
                type="number"
                value={shiftHours}
                onChange={(e) =>
                  setShiftHours(clamp(parseFloat(e.target.value || "0"), 1, 24))
                }
              />
              <div className="text-xs text-muted-foreground">
                Used to compute planned pace and takt.
              </div>
              <div className="text-xs text-muted-foreground">
...
export default function DepartmentSupervisorOverviewHiFi() {
  ...
  const workstations = useMemo<WorkstationRow[]>(() => {
    // Exit rate is CURRENT throughput for the most recent report window (intervalMinutes), shown as units/hr.

    // Department throughput for the current interval (demo approximation)
    const plannedPerInterval = goalUnits / intervalCount;
    const x = asOfIdx / Math.max(1, intervalCount - 1);
    const dip = Math.exp(-Math.pow((x - 0.55) / 0.16, 2));
    const noise = 0.04 * Math.sin(x * Math.PI * 2.2 + 0.4);
    const wobble = 1.01 - 0.15 * dip + noise + 0.01 * x;

    let deptActualThisInterval = plannedPerInterval * wobble;
    if (asOfIdx === intervalCount - 1) deptActualThisInterval += 10;

    const baseShares = [0.18, 0.16, 0.17, 0.15, 0.19, 0.15];
    const sumShares = baseShares.reduce((s, v) => s + v, 0) || 1;
    ...
    // Tie CURRENT exit rate to cycle-time performance.
    const benchProfiles = baseShares.map((share, idx) => {
      const shareNorm = share / sumShares;
      ...
      const fullShiftTargetOut = Math.max(1, Math.round(goalUnits * shareNorm));
      return { shareNorm, avgCycle, slowCycle, perf, fullShiftTargetOut };
    });

    const totalWeight = benchProfiles.reduce((s, b) => s + b.shareNorm * b.perf, 0) || 1;

    return benchProfiles.map((b, idx) => {
      const weight = (b.shareNorm * b.perf) / totalWeight;

      // Cumulative actual OUT to report time
      const out = Math.round(deptOutSoFar * weight);

      // Full-shift target OUT for the bench
      const targetOut = b.fullShiftTargetOut;

      // Current interval output -> current exit rate
      const outThisInterval = deptActualThisInterval * weight;
      const rate = outThisInterval / Math.max(0.001, hoursInInterval);
  ...
  // Progress marker: where each bench should be by the current report time if tracking plan.
  // Since targets are full-shift totals, the marker is the same fraction-of-shift for all benches.
  const planFrac = clamp((asOfIdx + 1) / intervalCount, 0, 1);
              ...
              <Table>
                ...
                <TableBody>
                  {workstations.map((w) => {
                    ...
                    // Planned position (target so far) for this bench at the current report time
                    const targetNow = Math.round(clamp(w.targetOut * planFrac, 0, w.targetOut));
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            ...
                            <div className="relative w-full">
                              ...
                              <div
                                className="absolute top-0 -bottom-[3px] w-[2px] bg-emerald-500/90 shadow-[0_0_0_2px_rgba(255,255,255,0.95)]"
                                style={{ left: `${clamp(planFrac * 100, 0, 100)}%` }}
                              />
                            </div>
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                              <span>{w.out} / {w.targetOut}</span>
